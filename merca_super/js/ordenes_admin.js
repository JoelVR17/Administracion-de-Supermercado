$(document).ready(function () {
    ...
    tablaCompras = $('#tablaCompras').DataTable({
        "ajax": {
            "url": "database/ordenes_admin.php",
        ...
        "columns": [
            ...
            { "data": "monto" },
            { "data": "fecha_orden" },
            { "defaultContent": "<div <button class='borrarBtn btn-sm mr-3 rounded btnBorrar' title='Borrar orden compra'><i class='material-icons'>delete</i></button></div></div>" }
        ],
    ...
    $(document).on("click", ".btnBorrar", function () {
        fila = $(this);
        cliente = parseInt($(this).closest('tr').find('td:eq(0)').text());
        opcion = 1; //Eliminar    
        // Uso de Sweet Alert 2 para mostrar una alerta al borrar una consulta    
        Swal.fire({
            title: 'Esta seguro de borrar esta orden compra?',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#2b9348',
            cancelButtonColor: '#d00000',
            confirmButtonText: 'Si, borrar orden compra',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function () {
                        tablaCompras.row(fila.parents('tr')).remove().draw();
                    }
                });
                Swal.fire(
                    'Orden compra borrada!',
                    `La orden de compra ha sido borrada`,
                    'success'
                )
            }
        })
